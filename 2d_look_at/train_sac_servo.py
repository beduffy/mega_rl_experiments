...
class ReplayBuffer:
    def __init__(self, capacity=100000, device="cuda"):
        ...
        self.images = torch.zeros(
            (capacity, 3, 84, 84), dtype=torch.uint8, device=device
        )
        self.next_images = torch.zeros(
            (capacity, 3, 84, 84), dtype=torch.uint8, device=device
        )
    ...
    def _push_to_device(self, image, qpos, action, reward, next_image, next_qpos, done):
        ...
        self.actions[self.pos] = torch.from_numpy(
        ...
        if self.device == "cuda":
            with torch.cuda.stream(torch.cuda.Stream()):
                self._push_to_device(
                    image, qpos, action, reward, next_image, next_qpos, done
                )
        else:
            self._push_to_device(
                image, qpos, action, reward, next_image, next_qpos, done
...
class Critic(nn.Module):
    def __init__(self, image_size=240, qpos_dim=2, action_dim=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                3, 8, kernel_size=8, stride=4
            ),  # Bigger stride to reduce dimensions faster
            nn.ReLU(),
            nn.Conv2d(8, 16, kernel_size=4, stride=2),
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=4, stride=2),
...
class SAC:
    ...
    def select_action(self, state, evaluate=False):
        ...
        image = (
            torch.FloatTensor(np.transpose(image, (2, 0, 1)))
            .unsqueeze(0)
            .to(self.device)
            / 255.0
        )
        qpos = (
            torch.FloatTensor(qpos).flatten().unsqueeze(0).to(self.device)
        )  # Ensure [1, 2] shape
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            ...
            q_target = (
                reward.unsqueeze(-1) + (1 - done.unsqueeze(-1)) * self.gamma * q_next
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.2, help="Temperature parameter"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log-dir", type=str, default="runs", help="Directory for tensorboard logs"
    )
...
def main():
    ...
    # Add detailed timing
    timings = {"env_step": 0, "select_action": 0, "buffer_push": 0, "train_step": 0}
    ...
    print(
        f"Total: {count_parameters(agent.policy) + count_parameters(agent.critic1) * 2:,}"
    )
        ...
        if episode % args.eval_interval == 0:
            ...
            for _ in range(args.eval_episodes):
                ...
                while True:
                    ...
                    eval_next_state, eval_r, eval_done, _ = eval_env.step(
                        eval_action.cpu().numpy()[0]
                    )
                    eval_reward += eval_r
