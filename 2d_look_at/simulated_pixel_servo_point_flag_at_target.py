...
class ServoEnv:
    ...
    def _sample_target_position(self) -> Tuple[int, int]:
        ...
        distance = np.random.uniform(
            self.servo_radius + self.flag_length,
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        self.state.current_angle = (self.state.current_angle + np.pi) % (
            2 * np.pi
        ) - np.pi
    ...
    def _render(self) -> None:
        ...
        # Draw servo base
        pygame.draw.circle(
            self.screen, self.BLACK, self.servo_center, self.servo_radius
        )

        # Draw flag (line)
        flag_end = (
            self.servo_center[0]
            + int(self.flag_length * np.cos(self.state.current_angle)),
            self.servo_center[1]
            + int(self.flag_length * np.sin(self.state.current_angle)),
        )
    ...
    def save_recording(self, filename: str) -> None:
        ...
        try:
            with h5py.File(filename, "w") as f:
                ...
                # Save observations
                f.create_dataset(
                    "/observations/images/main", data=images, compression="gzip"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--policy_path",
        type=str,
        default="servo_policy.pth",
        help="Path to trained policy weights",
    )
    parser.add_argument(
        "--output", type=str, default=None, help="Output filename for recording"
    )
    parser.add_argument(
        "--render-mode",
        type=str,
