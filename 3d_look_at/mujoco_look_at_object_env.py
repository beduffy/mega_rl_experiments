...
class LookAtObjectEnv(gym.Env):
    ...
    def __init__(self, render_mode="human"):
        ...
        self.camera_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_CAMERA, "rgb_camera"
        )
        self.camera_body_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "camera_body"
        )
    ...
    def reset(self, seed=None, options=None) -> Tuple[np.ndarray, dict]:
        ...
        self.data.qpos[camera_qpos_start : camera_qpos_start + 3] = [
            0,
            0,
            2,
        ]  # Position
        self.data.qpos[camera_qpos_start + 3 : camera_qpos_start + 7] = [
            1,
            0,
            0,
            0,
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if action == CameraAction.YAW_LEFT:
            ...
            self.data.qpos[quat_start : quat_start + 4] = [
                new_quat[3],
                new_quat[0],
                new_quat[1],
                new_quat[2],
            ...
            self.data.qpos[pos_start : pos_start + 3] += (
    ...
    def get_observation(self) -> Tuple[np.ndarray, Optional[float]]:
        ...
        self.renderer.update_scene(
            self.data
        )  # Remove camera parameter to use default view
