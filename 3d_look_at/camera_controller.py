...
class CameraController:
    ...
    def get_camera_image(self):
        ...
        if self._last_params != current_params:
            ...
            self._target_position[0] = self.camera_position[0] + np.cos(
                yaw_rad
            ) * np.cos(pitch_rad)
            self._target_position[1] = self.camera_position[1] + np.sin(
                yaw_rad
            ) * np.cos(pitch_rad)
        ...
        np.copyto(
            self._rgb_buffer,
            np.frombuffer(px, dtype=np.uint8).reshape(self.height, self.width, 4)[
                :, :, :3
            ],
        )
